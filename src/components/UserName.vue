<template>
    ...
    <div class="max-w-[425px] mx-auto px-4">
        <input type="text" v-model="items" class="inline-block w-full p-3 outline-none text-[#060921]"
            placeholder="Add Item" @keyup.enter="addItem" required>
        <i class="fa-solid fa-plus ml-[-28px] text-xl text-[#060921] opacity-70 cursor-pointer transition-all duration-150 hover:opacity-100"
            @click="addItem"></i>
    </div>
    <ul class="max-w-[425px] mx-auto mt-8 px-4">
        <li class="bg-[#5535be] p-2 flex justify-between items-center mb-2" v-for="x in todos"><span
                ...
                class="fa-solid fa-trash text-white text-xl cursor-pointer opacity-70 transition-all duration-150 hover:opacity-100"
                @click="removeItem(todos.indexOf(x))"></i>
        </li>
    </ul>
</template>
...
export default {
    ...
    methods: {
        addItem() {
            if (this.items.trim() !== "") {
                this.items = this.items.charAt(0).toUpperCase() + this.items.slice(1);
